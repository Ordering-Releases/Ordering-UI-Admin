...
import { Logistics as LogisticsController, useUtils } from 'ordering-components-admin-external'
  ...
  const getEventName = (log) => {
    switch (log.event) {
      case 'logistic_driver_found':
        return `Driver ${log.driver.name} found to the order at ${parseDistance((log?.data?.distance / 1000).toFixed(2))}`
