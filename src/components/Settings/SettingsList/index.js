                    ...
                    <div key={i}>
                      ...
                      {config.type === 6 && (
                        <>
                          <FormGroupText className='form-group'>
                            <label>{config?.name}</label>
                            {config?.description && <Description>{config?.description} {[1312, 1315, 1316].includes(config?.id) && `(${t('MINUTES', 'Minutes')})`}</Description>}                            {config?.value?.split('|')?.filter(value => valuesTypeSix.includes(value?.split(',')[0]))?.map((value, i, hash) => (
