...
const ReportsGeneralSalesUI = (props) => {
  const {
    filterList,
  ...
  const downloadCSV = () => {
    ...
    downloadLink.download = `general_sales_${fileSuffix}.csv`
  ...
  return (
    <ReportsBusinessSpendContainer>
      <Title>{t('GENERAL_SALES', 'General sales')}</Title>
      <ButtonActionList>
        <BrandBusinessWrapper>
          <Button
      ...
      <DistancePerBrandWrapper>
        <DistanceTitleBlock active={reportData?.content?.body?.rows?.length > 0}>
          <h2>{t('GENERAL_SALES', 'General sales')}</h2>
...
export const ReportsGeneralSales = (props) => {
