...
import { LinkableText, AutoScroll, Alert, NotFoundSource } from '../../../Shared'
...
import { Button, Tabs, Tab, Input, TextArea } from '../../../../styles'
...
const ProductOptionsUI = (props) => {
  ...
  const handleChangeTabValue = (value) => {
    if (document.getElementById(`${value}`)) {
      ...
      const scrollElement = document.querySelector('.dialog-inner')
        ...
        <>
          <ProductInfo>
            ...
            <ProductEdition>
              ...
              {product?.ingredients?.length > 0 && Object.keys(productCart.ingredients).length > 0 && (
                <div id='ingredients'>
            ...
            <ProductActions isColumn={(!(orderState.options?.address_id || unaddressedTypes.includes(orderState?.options?.type)))}>
              {(actionStatus?.loading || orderState.loading) ? (
                ...
                <>
                  ...
                  {productCart && !isSoldOut && maxProductQuantity > 0 && (orderState.options?.address_id || unaddressedTypes.includes(orderState?.options?.type)) && (
                    <Button
                      borderRadius='8px'
